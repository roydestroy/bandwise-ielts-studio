...
- [x] **Limit spend** (built 2026-09-26). Every Bandwise AI run checks a monthly credit allowance per workspace (`lib/ai-credits.ts`; 150 by default, `PLATFORM_MONTHLY_CREDITS`, or a per-teacher override in **Admin panel → Usage and pricing**). Out of credits returns HTTP 402 with the renewal date. Per-call output is already capped (11,000 tokens for an assessment, with Gemini 2.5 thinking at a third of that), so one call costs at most about $0.03; the allowance caps the volume. Still to do outside the code: a monthly budget alert in Google Cloud Billing.
