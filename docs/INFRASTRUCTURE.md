...
| Custom domains | **`bandwiseapp.com`** (canonical), `www.bandwiseapp.com`, `bandwise.eurognosi-remote.com` | `wrangler.json` → `routes` (first route = canonical, exported as `CANONICAL_HOST`) | From the domain release: the Worker (`worker/index.ts`) redirects every hostname except `bandwiseapp.com` to it, keeping path and query (301, or 308 for non-GET). `bandwiseapp.com` was bought on 2026-09-26 through Cloudflare Registrar in this account. Previously the only domain was `bandwise.eurognosi-remote.com`. |
...
| `/app/admin` | Admins (`ADMIN_EMAILS`) | The **Admin panel**: approve or pause accounts, usage and pricing, accuracy, pilot feedback. |
