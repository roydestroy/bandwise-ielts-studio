...
async function report(owner:string,teacher:string,body:unknown){
  ...
  const rows=await db.prepare('SELECT * FROM assessments WHERE owner = ? AND student_id = ?').bind(owner,id).all();
  return {student,report:buildProgressReport({student,assessments:rows.results.map(parseRow),teacher,note})};
}
...
export async function POST(req:Request){
  ...
  try{
    ...
    if(body.action==='preview'){const {student,report:r}=await report(owner,teacher,body);return json({html:r.html,subject:r.subject,to:student.email,empty:r.empty});}
    if(body.action==='send'){
      const {student,report:r}=await report(owner,teacher,body);
