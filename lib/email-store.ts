...
export async function saveMail(owner:string,config:MailConfig,password?:string){
  ...
  const secret=(password&&cleanSecret(password))||(await credentials(owner).catch(()=>null))?.password;
...
function explain(e:unknown,nonAscii:boolean){
  const m=e instanceof Error?e.message:String(e);
  if(/auth|535|534|credential|password/i.test(m))return 'The mail server rejected the username or password. Many providers need an app password instead of your normal one.'+(nonAscii?' Your saved password or username contains non-English characters — check your keyboard layout (app passwords use only English letters) and enter it again.':'');
